    ...
    <section class="container mx-auto">
      <div class="bg-white rounded border border-gray-200 relative flex flex-col">
        <div class="px-6 pt-6 pb-5 font-bold border-b border-gray-200"
          ...
          <span class="card-title">{{ $t('songs_label') }}</span>
          <!-- Icon -->
        </div>
        <!-- Playlist -->
        <ol id="playlist">
...
export default {
  ...
  methods: {
    async getSongs() {
      ...
      let snapshots;
      let lastSongRequested;
      if (!this.allSongsLoaded) {
        if (this.songs.length) {
          lastSongRequested = this.songs[this.songs.length - 1].docID;
      ...
      if (this.songs[this.songs.length - 1].docID === lastSongRequested) {
