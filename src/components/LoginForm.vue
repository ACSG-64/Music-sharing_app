<template>
  <div class="text-white text-center font-bold p-4 mb-4"
       v-show="login_show_alert" :class="login_alert_variant">
    {{ login_alert_msg }}
  </div>
  <vee-form :validation-schema="loginSchema" @submit="login">
    <!-- Email -->
    <div class="mb-3">
      <label class="inline-block mb-2">{{ $t('login_form.email_label') }}</label>
    ...
    <!-- Password -->
    <div class="mb-3">
      <label class="inline-block mb-2">{{ $t('login_form.password_label') }}</label>
...
export default {
  ...
  data() {
    return {
      ...
      login_alert_msg: 'Please wait, logging in process.',
    };
  },
  methods: {
    async login(values) {
